$( document ).ready( function () {
  ...
  $( 'nav>ul>li, #user_info, .filterBar>ul>li' ).hoverIntent( function () {
  ...
  $( '.cheat_sheet_row' ).hoverIntent( function () {
  ...
  $( '#cs_filter' ).keyup( function ( e ) {
    ...
    if ( filter.length > 0 ) {
      ...
      $( '.cheat_sheet_output_block tr' ).find( "td:icontains(" + filter + ")" ).parent().show();
      $( ".cheat_sheet_output_wrapper h3:icontains(" + filter + ")" ).parent().find( "tr" ).show();
      $( '.cheat_sheet_output_block .cheat_sheet_note' ).hide();
      $( ".cheat_sheet_output_block .cheat_sheet_note:icontains(" + filter + ")" ).show();
      if ( !searchEventFired ) {
        ga( 'send', 'event', 'Cheat Sheets', 'Cheat Sheet Searched', cheatSheetName );
        if ( bTracking ) {
          ga( 'userTracker.send', 'event', 'Cheat Sheets', 'Cheat Sheet Searched', cheatSheetName );
        }
        searchEventFired = true;
      }
      searchTermTimeout = setTimeout( function () {
        ga( 'send', 'event', 'Cheat Sheets', 'Cheat Sheet Search Term (' + cheatSheetName + ')', filter );
        if ( bTracking ) {
          ga( 'userTracker.send', 'event', 'Cheatography Cheat Sheets', 'Cheat Sheet Search Term (' + cheatSheetName + ')', filter );
        }
      }, 500 );
    } else {
    ...
    $( ".cheat_sheet_output_block" ).each( function ( index, element ) {
      var counter = 0;
      $( element ).find( 'tr' ).not( ':hidden' ).each( function ( inner_index, inner_element ) {
        counter += 1;
        if ( ( counter % 2 ) == 1 ) {
  ...
  $( '.content_popup' ).fancybox( {
    ...
    'beforeShow': function () {
      ...
      if ( pageUrl.substring( 0, 54 ) == 'http://www.cheatography.com/proxy.php?mode=native&url=' ) {
      ...
      if ( bTracking ) {
        ga( 'userTracker.send', 'event', 'Cheatography Cheat Sheets', 'Content Popup', pageUrl );
      }
    },
    'titleShow': false
  } );
  ...
  processTags();
  $( 'input.tagoptions, input.langoptions, input.ratingoptions' ).click( function () {
    processTags();
  } );
  $( '#filterTags' ).keyup( function () {
    var filter = $( this ).val();
    if ( filter.length > 0 ) {
      $( '#filterTagsList li.filterableItem .tagname:icontains(' + filter + ')' ).parent().parent().show();
      $( '#filterTagsList li.filterableItem .tagname' ).not( ':icontains(' + filter + ')' ).parent().parent().hide();
    } else {
  ...
  $( '#filterLangs' ).keyup( function () {
    var filter = $( this ).val();
    if ( filter.length > 0 ) {
      $( '#filterLangsList li.filterableItem .tagname:icontains(' + filter + ')' ).parent().parent().show();
      $( '#filterLangsList li.filterableItem .tagname' ).not( ':icontains(' + filter + ')' ).parent().parent().hide();
    } else {
  ...
  $( '.ratingStars' ).each( function () {
    ...
    $( ratingParent ).children( 'i' ).each( function ( index ) {
      if ( loggedIn ) {
        $( this ).hover( function () {
          ...
          for ( var i = 1; i <= 5; i++ ) {
            if ( i <= ratingValue ) {
              $( ratingParent ).children( 'i' ).eq( i - 1 ).attr( 'class', 'fa fa-star' );
            } else {
              $( ratingParent ).children( 'i' ).eq( i - 1 ).attr( 'class', 'fa fa-star-o' );
            }
          }
        }, function () {
          if ( !hasUserRating ) {
        ...
        $( this ).click( function () {
          var ratingValue = index + 1;
          $.ajax( {
            url: '/ajax.php?action=rate_cheat_sheet&rating=' + ratingValue + '&cheat_sheet_id=' + ratableCheatSheetId,
            success: function ( response ) {
              ...
              if ( !hasUserRating ) {
                if ( ratingValue <= 2 ) {
                  alert( 'Thanks for rating this cheat sheet! Please consider leaving a comment for the author to let them know how they can improve their cheat sheet.' );
                } else {
        ...
        $( this ).click( function () {
          alert( 'Please login or register to rate cheat sheets and organise your favourites!' );
        } );
      }
      for ( var i = 1; i <= 5; i++ ) {
  ...
  if ( deviceBracket == 'desktop' ) {
    $( '.footer_popup_inner' ).append( '<a href="#" class="imagelink footer_popup_close"><i class="fa fa-close"></i></a>' );
...
function processTagsOld() {
  ...
  $( 'input.tagoptions' ).not( ':checked' ).each( function () {
    $( this ).parent().css( 'background-color', 'inherit' );
  } );
  $( 'input.langoptions' ).not( ':checked' ).each( function () {
    $( this ).parent().css( 'background-color', 'inherit' );
  } );
  $( 'input.ratingoptions' ).not( ':checked' ).each( function () {
    $( this ).parent().css( 'background-color', 'inherit' );
  } );
  if ( !tagFilterFound ) {
    $( '#filtering' ).html( '' );
  ...
  if ( !filterFound ) {
    ...
    $( '#content .cheat_sheet_row' ).removeClass( 'hiderow' );
    if ( to_show != '' ) {
      $( '#content .cheat_sheet_row' ).not( to_show ).addClass( 'hiderow' ).hide( 'slow' );
    }
    if ( to_show_lang != '' ) {
      $( '#content .cheat_sheet_row' ).not( to_show_lang ).addClass( 'hiderow' ).hide( 'slow' );
    }
    if ( to_show_rating != '' ) {
      $( '#content .cheat_sheet_row' ).not( to_show_rating ).addClass( 'hiderow' ).hide( 'slow' );
    }
...
function trackDownload( downloadType, newLocation ) {
  ga( 'send', 'event', 'Cheat Sheets', 'Download', cheatSheetName + ' (' + downloadType + ')' );
  if ( bTracking ) {
    ga( 'userTracker.send', 'event', 'Cheatography Cheat Sheets', 'Download', cheatSheetName + ' (' + downloadType + ')' );
  }
...
function trackDownloadPopup() {
  ga( 'send', 'event', 'Cheat Sheets', 'Download Popup', 'Opened Download Popup for ' + cheatSheetName );
  if ( bTracking ) {
    ga( 'userTracker.send', 'event', 'Cheatography Cheat Sheets', 'Download Popup', 'Opened Download Popup for ' + cheatSheetName );
  }
}

function trackRatingPopup() {
  ga( 'send', 'event', 'Interaction', 'Rating Popup', 'Opened Rating Popup for ' + cheatSheetName );
}
...
function setCsOrder() {
  ...
  $( '.cheat_sheet_output_column_1 .cheat_sheet_output_wrapper' ).each( function () {
    var blockId = $( this ).attr( 'id' );
    blockId = blockId.substring( 6 );
    csOrder.cs1.push( blockId );
  } );
  $( '.cheat_sheet_output_column_2 .cheat_sheet_output_wrapper' ).each( function () {
    var blockId = $( this ).attr( 'id' );
    blockId = blockId.substring( 6 );
    csOrder.cs2.push( blockId );
  } );
  $( '.cheat_sheet_output_column_3 .cheat_sheet_output_wrapper' ).each( function () {
    var blockId = $( this ).attr( 'id' );
    blockId = blockId.substring( 6 );
    csOrder.cs3.push( blockId );
  } );
  $( '.cheat_sheet_output_column_4 .cheat_sheet_output_wrapper' ).each( function () {
    var blockId = $( this ).attr( 'id' );
    blockId = blockId.substring( 6 );
...
function checkContrast( value, element ) {
  ...
  var contrast = ( 1.05 / ( 0.05 + ( 0.2126 * Math.pow( rdec / 255, 2.2 ) ) + ( 0.7152 * Math.pow( gdec / 255, 2.2 ) ) + ( 0.0722 * Math.pow( bdec / 255, 2.2 ) ) ) );
  if ( contrast < 1.5 ) {
    return false;
  } else {
    return true;
